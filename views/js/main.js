...
var resizePizzas = function (size) {
	...
	//removed the below variables from the determineDx() function because they were already accessible outside of the function.
	//Only need to call the sizeSwitcher function.  
	var newSize = sizeSwitcher(size);
	var oldSize = offsetWidth / windowWidth; //offsetWidth equaled the oldWidth, so eliminated oldWidth.
	var dx = (newSize - oldSize) * windowWidth;
	var newwidth = (offsetWidth + dx) + 'px';
	//make an array of all elements with the "randomPizzaContainer" class:
	var allPizzas = document.getElementsByClassName("randomPizzaContainer");
	// Iterates through pizza elements on the page and changes their widths
	//cut this function down to only needing to update the newwidth property
...
function updatePositions() {
	...
	//changed to making an array by className instead of SelectorAll
	var movers = document.getElementsByClassName('mover'); //collects all mover elements total of 24
	//took scrollTop out of the for loop below as it is a constant at call time.
	var scrollTop = (document.documentElement.scrollTop || document.body.scrollTop) / 1250;
	//adjusts the left propert according to one of 5 phase values determined by using modulus of 5; the remainders can only be 0 to 4
	...
	for (var i = 0; i < movers.length; i++) {
		movers[i].style.transform = "translateX(" + (phaseArray[i % 5]) + "px)";
	}
...
document.addEventListener('DOMContentLoaded', function () {
	...
	for (var i = 0; i < PizzaOrderSize; i++) {
		...
		elem.style.left = (i % cols) * s + "px"; //changed to left from basicleft as left will not be updated with transform()
