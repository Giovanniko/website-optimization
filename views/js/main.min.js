...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");var movers=document.getElementsByClassName('mover');var scrollTop=(document.documentElement.scrollTop||document.body.scrollTop)/1250;var phaseArray=[];for(var i=0;i<5;i++){phaseArray[i]=Math.sin((scrollTop)+(i%5))*100}
for(var i=0;i<movers.length;i++){movers[i].style.transform="translateX("+(phaseArray[i%5])+"px)"}
...
window.addEventListener('scroll',updatePositions);document.addEventListener('DOMContentLoaded',function(){var cols=screen.width/250;var rows=screen.height/225;var PizzaOrderSize=Math.floor(rows*cols);var s=256;for(var i=0;i<PizzaOrderSize;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.style.left=(i%cols)*s+"px";elem.style.top=(Math.floor(i/cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem)}
